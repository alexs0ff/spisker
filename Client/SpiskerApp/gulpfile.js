...
var prodContext = {
  destDir: paths.dest,
  indexPage: "./ClientApp/static/index-aot.html"
};
...
gulp.task("dev:prepare", function (cb) {
  return builddev.prepareToLaunch(devContext);
});
...
gulp.task("dev:webpack", function (cb) {
  return buildwebpack.build(devContext);
});
...
gulp.task("prod:webpack", function (cb) {
  return buildwebpack.build(packProdContext);
});
