...
compilation.webpack = function (configFilePath) {
    gutil.log("Webpack compiling  with " + gutil.colors.magenta(configFilePath));

    return gulp.src(configFilePath).pipe(compilation.execute('"node_modules/.bin/webpack" --config ' + configFilePath+ ' --bail'))
